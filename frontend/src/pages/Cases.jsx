  ...
  return (
    <div className="space-y-4">
      ...
      <div className="card p-5">
        ...
        <p className="mt-1 text-sm text-mute">{tr(c.category)} · {tr(c.priority)} · {c.jurisdiction}</p>
        {c.summary && <p className="mt-2 text-sm text-ink">{c.summary}</p>}
        <div className="mt-3 flex flex-wrap gap-2">
          {STATUSES.map((s) => (
            <button key={s} onClick={() => setStatus(s)}
              className={`chip ${c.status === s ? 'bg-teal text-white' : 'bg-slate-100 text-mute'}`}>{tr(s)}</button>
          ))}
        </div>
      </div>
      ...
      <div className="card p-5">
        ...
        <div className="mt-3 flex gap-2">
          <input className="field flex-1" placeholder={tr('Add a note…')} value={note}
    ...
    <div className="grid gap-5 lg:grid-cols-3">
      <div className="space-y-5 lg:col-span-2">
        ...
        <section className="card grid gap-3 p-5 sm:grid-cols-2">
          <input className="field sm:col-span-2" placeholder={tr('Case title')} value={form.title}
            onChange={(e) => setForm({ ...form, title: e.target.value })} />
          <select className="field" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
            {['tenancy', 'employment', 'consumer', 'wages', 'other'].map((c) => <option key={c} value={c}>{tr(c)}</option>)}
          </select>
          <select className="field" value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}>
            {['low', 'medium', 'high', 'urgent'].map((c) => <option key={c} value={c}>{tr(c)}</option>)}
          </select>
          <textarea className="field sm:col-span-2" placeholder={tr('Summary')} value={form.summary}
          ...
          {rows.map((c) => (
            <button key={c.id} onClick={() => setOpen(c.id)} className="card flex w-full items-center gap-3 p-4 text-left hover:shadow-md">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-mute">{tr(c.category)} · {tr(c.priority)}</div>
              </div>
              <span className={`chip ${STATUS_COLOR[c.status]}`}>{tr(c.status)}</span>
            </button>
          ))}
